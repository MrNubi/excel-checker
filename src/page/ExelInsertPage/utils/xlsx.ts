...
export function validateRows(rows: Row[]) {
  ...
  rows.forEach((r, i) => {
    ...
    // 필수값
    REQUIRED.forEach((key) => { if (isUnknownLike(r[key])) u.push({ field: key, value: r[key] }); });
    ...
    if (!Number.isFinite(amount) || amount < 0) u.push({ field: "금액", value: r["금액"] });
...
export function compareByCompositeKey(A: Row[], B: Row[]): CompareResult {
  ...
  for (const [k, r] of mapA) {
    if (mapB.has(k)) matches.push(r); else onlyInA.push(r);
  }
